...
def save_ply(frame):
    ...
    global counter
    ply = rs.save_to_ply("%s.ply" % (counter))

    ply.set_option(rs.save_to_ply.option_ply_binary, False)
    ply.set_option(rs.save_to_ply.option_ply_normals, True)

    print("Saving to %s.ply ..." % counter)
...
if __name__ == '__main__':
    model = YOLO("best.pt")  # 加载权重文件，如需要更换为自己训练好的权重best.pt即可
    ...
    try:
        while True:
            ...
            depth_intrin, img_color, aligned_depth_frame = get_aligned_images(frames)  # 获取对齐图像与相机参数
            # 检查是否达到间隔时间
